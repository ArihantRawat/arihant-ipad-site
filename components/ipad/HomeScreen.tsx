...
function AppIcon({
  ...
  return (
    ...
    >
      ...
      >
        {app.id === "work" ? (
          <WorkCalendarIcon size={size} now={now} />
        ) : app.icon ? (
          ...
          >
            {gridItems.map((app, i) =>
              app === null ? (
                ...
                <AppIcon key={app.id} app={app} size={iconSize} now={now} onTap={(e) => handleOpen(app, e)} />
              )
            )}
          </motion.div>
        </div>
